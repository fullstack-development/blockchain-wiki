...
При всей простоте основной идеи вестинга (блокировка токенов после покупки с последующей разблокировкой по графику), техническая реализация может очень отличаться от проекта к проекту. Фантазия менеджеров продукта и принципы децентрализации диктуют свои правила, поэтому перед тем как реализовать его на смарт-контрактах нужно ответить на ряд вопросов.
...
2. Как токены будут блокироваться? Во время токенсейла или для управляющими проекта? (к примеру вестинг для разработчиков или маркетологов).
...
1. Считается хорошей практикой код смарт-контракта токена делать "чистым", чтобы он был простым и понятным, любая дополнительная логика требует тщательных проверок, а в идеале хороших аудитов.
...
2. Используется подход выплат pull, а это значит, что бенефициару потребуется выполнить две транзакции: покупка и клейм.
...
**Контракт приватных продаж.** Его основная задача продажа токенов через вайтлист. Также он отвечает за управление параметрами продажи, прием платежей, но самое важное для нас в данном случае - он отвечает за чеканку share-токенов покупателям. Вся фишка в том, что в момент продажи мы переводим покупаемые токены не пользователю, а на смарт-контракт share-токена, где они блокируются, пользователю в свою очередь чеканятся share-токены.
Пример такого смарт-контракта я здесь приводить не буду, т.к. фактически это может быть даже обычный EOA, все от него потребуется - выдать апрув на базовый токен для share-токена после чего можно будет его минтить.
...
Такой подход позволяет создавать несколько share-токенов под разные раунды продаж, у каждого будет свое расписание вестинга плюс он будет выступать пулом для базовых токенов с раунда продаж. Такой подход дал даже больше гибкости чем мы думали изначально, но об этом чуть позже.
