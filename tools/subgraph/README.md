...
- Graph Node находит событияблокчейн для вашего subgraph в этих блоках и запускает предоставленные вами обработчики сопоставления(mapping handlers). Обработчик — это модуль WASM, который создает или обновляет объекты данных, которые Graph Node хранит в ответ на события блокчейн. 
...
```
На данный момент, у нас должны быть сущности как в этой [схеме](./schema.graphql). Можете поправить манифест и обновить там сущности. После написания, запусти команду генерации типов - graph codegen , позже расскажу подробней для чего это нужно.
...
Попробуйте сами дописать остальные 3 функции. Если не получится - [вот](./wiki.ts) заполненный пример.
...
Тут будет теоретическая часть не связанная нашим кодом. Вы можете поэксперементировать и написать такой пример для настоящего uniswap.
