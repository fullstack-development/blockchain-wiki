...
contract SingleHTLC {
    ...
    function _getRecipientOrder(address sender, bytes memory secret) private view returns (Order memory) {
        bytes32 secretHash = keccak256(abi.encodePacked(secret));
        Order memory senderOrder = _lockedOrders[sender][secretHash];

        return _lockedOrders[senderOrder.recipient][secretHash];
    }
