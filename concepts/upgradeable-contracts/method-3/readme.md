...
Шаблон прокси использует разделение данных для хранения бизнес-логики и состояние в отдельных контрактах. Однако шаблон прокси отлчиается от способа 2(разделение данных). Это обратный метод разделения данных, когда контракт хранилище вызывает логический контракт.
...
_Очень важно!_ Этот подход не служит для обновления логики контрактов задеплоенных при помощи **eip-1167**. Это просто дешевый, контролируемый способ создать клона существуюещего контракта.
...
Плагин поддерживают шаблоны прокси UUPS, Transparent и Beacon. В зависимости от шалона варианты обновления отличаются.

## OpenZeppelin utils

Как мы говорили выше ugradeable контракты не имеют ```constructor()```. Вместо этого используется общепринятая функция ```initialize()```. Напоминаем, что она используется для первичной инициализации данных при деплое обновляемого контракта.
...
_Важно!_ Помимо того, что нельзя оставлять proxy контракт неинициилизированым, также не рекомендуется оставлять возможность вызвать функцию ```initialize()``` на контракте логики.
