...
Имея хороший инструмент для обмена, я хочу делать обмены с наибольшей выгодой. Я не хочу ждать или искать обменник с подходящей ценой токена. Было бы здорово, если бы обменник умел "играть в такие игры". Я хочу получать от обменника профессиональные инструменты торговли из коробки:
 - отложенный обмен
 - обмен с дополнительными обьёмами средств
...
2. Он видит рыночную стоимость токена и понимает, что для него в данный момент это дорого. Особено, если учитывать, что он собирается купить сразу много.
...
5. После подверждения **ордер** попадает в **ордербук** и становится доступным для исполнения любым пользователем.
...
Линии снизу и сверху реального тела называются **тенями** или **фитилями** (shadows or wick). Каждая тень обозначает самую высокую или самую низкую цену, предложенную в выбраном периоде времени.
...
Ответ прост. Пользователю необязательно ориентироваться, ему достаточно создать свой ордер, а обменник автоматически проведёт матчинг ордера пользователя с отложенными ордерами.
...
Этот пошаговый алгорит можно представить в виде схемы.
...
> Например, пользователь хочет продать 2 ETH по цене 1600 USDT. В данный момент ETH cтоит гораздо меньше 1600 USDT. Пользователь создаёт отложенный ордер на продажу 2 ETH. Ордер отправляется в ордербук.
>
