...
Структура Uniswap V2 состоит из ядра и переферийных контрактов.
...
В нем будут реализованы следующие возможости:
...
Uniswap v3 основан на тех же принипах AMM c формулой постоянного значения произведения, что и ранние версии, но предлагает несколько нововведений.
...
Проблема в том, что большинство активов торгуются в определённых диапазонах. Больше всего это заметно в пулах со стейблкойнами, у которых диапазон очень узкий.
...
Первоначальная идея этого протокола — объединить большое количество стейблокойнов, чтобы минимизировать проскальзывание и защитить поставщиков ликвидности от непостоянных потерь.
...
1. **Взвешанные пулы**. Предназначены для общих случаев и не обязательно имеют ценовую корреляцию. Например, DAI/WETH. Цена определяется балансом пула, его весом и количеством обмениваемых токенов.
...
В действительности, это один большой пул ликвидности с кучей активов. За счёт этого снижается ценовое влияние при свапах больших объёмов активов, а также уменьшается проскальзывание.
...
Для предотвращения проблем, которые могут произойти с токенами, а тем более со стейблкоинами, основной пул содержит в себе лишь самые ликвидные и проверенные стейблкоины: USDC, USDT и BUSD. А также самый удачный на данный момент децентрилизованный стейблкоин со сверхобеспечением — DAI.
