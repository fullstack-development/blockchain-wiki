...
Дальше самое интересно - необходимо эти настройки применять на OApps, то есть на смарт-контрактах токенов в выбранных сетях и на контрактах Endpoint. Для этого есть отдельный скрипт, он выполнит нужные транзакции в каждой сети по отдельности и транзакций будет много, поэтому убедитесь, что хватить средств на оплату газа.
