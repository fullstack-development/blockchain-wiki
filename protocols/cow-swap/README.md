...
*Пример 1: Пользователь продает 1 ETH за DAI через партнерский виджет (Partner Fee = 1%)*
...
* Параметры подписанного ордера:
  * Сумма продажи: 1 ETH
  * Комиссия: 0 ETH (все ордера теперь имеют нулевую подписанную комиссию)
  * Минимальная сумма покупки: 3,465 DAI (3,500 - 3,500 × 0.01), тут взяли комиссию партнера

* Исполнение ордера:
  * Решатель использовал 0.003 ETH на газ
  * Продано: 0.997 ETH
  * Получено: 3,525 DAI (решатель смог получить излишек в 60 DAI)
...
**Процесс выплаты партнерской комиссии:**
1. После внедрения параметров Partner Fee в виджет, интегратор получает право на комиссию за сделки, совершенные через его виджет.
2. Еженедельно запускается скрипт для расчета суммы комиссии на основе всех транзакций, выполненных через интеграцию виджета, с Partner Fee в appData.
3. Чистая партнерская комиссия (за вычетом сервисного сбора CoW Protocol, который по умолчанию составляет 50%) переводится на указанный Ethereum-адрес интегратора еженедельно.
4. Выплаты производятся только если накопленная комиссия составляет не менее 0.001 WETH.

Таким образом решатель получает комиссию с части излишков, а также через протокол в виде токена COW.
