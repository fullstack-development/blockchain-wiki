...
- Вместо того чтобы раздавать approve каждому контракту на каждый токен, ты один раз дал разрешение GPv2VaultRelayer через `setRelayerApprova`l, и если захочешь — можешь одной транзакцией его отозвать. Это удобнее, безопаснее и контролируемо
