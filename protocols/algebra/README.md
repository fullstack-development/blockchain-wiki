...
Algebra — это DEX-as-a-Service (DaaS), то есть аналог SaaS (Software-as-a-Service), только в децентрализованном мире. Это B2B-решение для DEX на базе AMM, которое позволяет развернуть обменник, core-функционал которого будет во многом схож с Uniswap v3. Основной упор сделан на концентрированную ликвидность и изменяемых комиссиях (в Uniswap v3 комиссии фиксированы).
...
Важная деталь: Uniswap v3 был выпущен под лицензией BUSL-1.1, которая запрещала коммерческое использование в производственной среде (production use) до 1 апреля 2023 года без разрешения Uniswap Labs. Под _production use_ в данном случае подразумевается деплой кода в mainnet для обслуживания пользователей и получения дохода. Однако просмотр, модификация и использование кода в некоммерческих целях - например, для тестирования или разработки - были разрешены.

Algebra объявила о запуске 15 декабря 2022 года, то есть за 3,5 месяца до истечения срока действия лицензии Uniswap v3. Но важно понимать, что именно подразумевается под "запуском". У Algebra нет собственного DEX - они лишь предлагают коробочное решение, основанное на коде Uniswap v3 с рядом доработок.

Таким образом, до окончания действия лицензии Uniswap Labs команда Algebra лишь заявляет о партнерстве с крупными DEX’ами (QuickSwap, Camelot, THENA), в то время как фактический запуск этих проектов в mainnet происходит сразу после 1 апреля 2023 года (когда закончилось действие лицензии).
...
С момента запуска протокол претерпел значительные изменения, но эта структура остается актуальной, поскольку одной из главных задач является сохранение совместимости с интерфейсами Uniswap v3. На сегодняшний день последняя версия протокола - Algebra Integral (Algebra v2), но перед ней был выпущен минорный апдейт - Algebra v1.9.

## В чем сила Uniswap v3?

Что входит в базовый функционал Uniswap v3? В чем заключается инновационность этого протокола и почему его имеет смысл использовать как "DEX-движок"? Почему есть смысл форкать этот протокол?

Для начала разберемся с недостатками Uniswap v2. Несмотря на то, что этот протокол стал настоящей революцией и заложил основу для всего DeFi, он имел ряд существенных недостатков:
...
**Как Uniswap v3 уменьшил влияние непостоянных потерь?**
...
Ликвидность используется для свопов и приносит комиссии только тогда, когда текущая цена находится внутри заданного диапазона. Если цена выходит за пределы диапазона, позиция становится "неактивной" (не участвует в торгах), но активы по-прежнему остаются в безопасности. Позиции создаются в виде NFT и могут свободно продаваться на маркетплейсах, создавая вторичный рынок ликвидности.
...
[Algebra v1](https://github.com/cryptoalgebra/AlgebraV1) — версия протокола, с которой все началось. Помимо базового функционала Uniswap v3, она предлагала три ключевые функции:

1. Динамические комиссии
2. Встроенный фарминг (настройка "из коробки")
3. Поддержка дефляционных токенов

### Динамические комиссии

В своем [whitepaper](https://algebra.finance/static/Algerbra%20Tech%20Paper-15411d15f8653a81d5f7f574bfe655ad.pdf) Algebra выделяет три основные группы участников рынка:

1. Обычные трейдеры
2. Арбитражеры (зарабатывают на разнице цен между DEX или пулами)
3. Провайдеры ликвидности (LP)
...
Задача механизма комиссий - найти баланс между интересами всех сторон. В Uniswap v3 это реализовано через выбор фиксированных комиссий (0.05%, 0.3%, 1%). Проблема в том, что это ведет к фрагментации ликвидности между пулами.
...
Algebra использует гибкую формулу, учитывающую все эти параметры, и позволяет удерживать всю ликвидность в рамках одного пула, не распыляя ее по разным пулам с разными комиссиями.
...
Под поддержкой дефляционных токенов подразумеваются случаи, когда количество токенов может изменяться в процессе перевода токенов. Это, например, токены, у которых:

- часть объема сжигается при каждом переводе;
- взимается комиссия при переводе;
- реализованы другие механизмы, влияющие на итоговое количество получаемых токенов.

Для корректной работы с такими активами в Algebra реализована отдельная функция -  
...
Она определяет фактическое количество токенов, поступивших в пул после всех удержаний и сгораний, что позволяет корректно провести своп.

В Uniswap v3 такой возможности нет - для поддержки дефляционных токенов требуется создание токенов-оберток, что усложняет взаимодействие с такими активами.

## Algebra v1.9

Перед выпуском второй версии протокола Algebra представила промежуточное обновление - [Algebra v1.9](https://github.com/cryptoalgebra/AlgebraV1.9). Это минорный релиз, сфокусированный на оптимизации контрактов, улучшении работы с тиками и добавлении новых возможностей. Рассмотрим ключевые изменения.
...
В начале статьи я упоминал, что для релиза первой версии протокола пришлось дождаться окончания срока действия лицензии BUSL-1.1 для Uniswap v3. Четвертую версию (Uniswap v4) команда Uniswap [анонсировала](https://blog.uniswap.org/uniswap-v4) в июне 2023 года. В этом анонсе было представлено новое видение - единый пул и модульная архитектура через хуки. Запуск Uniswap v4 состоялся только в январе 2025 года, новая версия протокола закрыта под лицензией BUSL до 15 июня 2027 года.

Команда Algebra Finance не стала перенимать идею единого пула, но реализовала модульность через хуки - на базе предыдущей версии, то есть архитектуры Uniswap v3. Отдельные модули в протоколе называются - плагины.

### Монолит vs модульность

Основная идея модульной архитектуры для DEX - создать простое, надежное и безопасное ядро, которое можно расширять с помощью внешних модулей (плагинов). Не каждому пулу с токенами необходимы, например, динамические комиссии или TWAP-оракул, а такой функционал может существенно увеличивать затраты на газ.
...
Главное здесь - обезопасить ликвидность, то есть сделать core-функционал закрытым от воздействия внешних модулей и неизменяемым. Такой подход, в теории, также позволяет добавлять новый функционал для DEX без необходимости миграции ликвидности.
